...
def mnist_gist(path, one_hot=True, **kwargs):
    """
    Inputs:
        path: The path containing mnist_gist512 features
        one_hot: If True, return one hoted labels.
    Outputs:
        train features, train labels, test features, and test labels.    
    """
    train_path = os.path.join(path,'mnist_gist512_train.mat')
    test_path = os.path.join(path,'mnist_gist512_test.mat')
...
def SUN397(path, one_hot=True, **kwargs):
    """
    Inputs:
        path: The path containing SUN397 features
        one_hot: If True, return one hoted labels.
    Outputs:
        train features, train labels, test features, and test labels.    
    """
    train_path = os.path.join(path,'sun397_vggfc7_gallery.mat')
    test_path = os.path.join(path,'sun397_vggfc7_test.mat')
    
    train_dict = loadmat(train_path, squeeze_me=True)
    test_dict = loadmat(test_path, squeeze_me=True)
    
    train_features, train_labels = train_dict['gallery_features'].T, train_dict['gallery_labels']
    test_features, test_labels = test_dict['test_features'].T, test_dict['test_labels']
...
Dataset_maps = {'cifar10_gist512':cifar10_gist, 'cifar10_vggfc7':cifar10_vggfc7,
                'mnist_gist512':mnist_gist, 'labelme_vggfc7':labelme_vggfc7, 
